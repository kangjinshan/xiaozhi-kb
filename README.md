...
| 最右 | BOOT | GPIO9 / BOOT 键 | 配置1单击为确定/回车；配置2单击显示触区图 |
...
触屏方向键和退格/回车触区支持按住重复发送，当前重复间隔约 120 ms。右下角右 Option 按触摸按住/松开处理。
...
3. 在屏幕上点击「蓝牙键盘 配置1」或「蓝牙键盘 配置2」。
4. 设备重启后进入蓝牙键盘模式。
5. 在 macOS 蓝牙设置中连接 `XiaoZhi KB`。
6. 连接成功后测试：
   - 最左键按住：右 Option。
   - 配置1：BOOT 单击回车，PWR 单击退格，触屏上下左右为方向键。
   - 配置2：BOOT 单击显示触区图，左下角退格，右上角回车，右下角按住右 Option。
...
- 点「小智语音」进入小智模式。
- 点「蓝牙键盘 配置1」或「蓝牙键盘 配置2」进入对应键盘模式。
