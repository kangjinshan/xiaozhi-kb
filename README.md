# XiaoZhi KB：小智语音 + 金山 AI 助手 + 蓝牙键盘/空鼠
...
- **蓝牙键盘/空鼠模式**：作为同一个 BLE HID 键鼠复合设备连接 MacBook；现有键盘配置直接叠加板载陀螺仪指针控制。
- **开机选择界面**：通过触摸屏在小智语音 / 金山 AI / 蓝牙键盘之间选择。

此外还为整机增加了 **SD 卡（TF 卡）支持**：三种模式均可挂载 SD 卡，图片和对话音频等文件可直接存卡。小智、金山 AI 和键鼠模式都不安装同步日志落盘钩子，避免任务栈溢出、SPI2 争用或 BLE 事件任务被 FATFS 阻塞；运行日志统一走 USB 串口。

项目当前已在真机上验证：SD 卡识别挂载、录音存 WAV、蓝牙键盘连接、触屏方向键、录音返回菜单稳定性、小智 Wi-Fi 启动稳定性，以及金山 AI 的 PWR 息屏/亮屏显示链路均正常。
...
- 触摸：CST9217，I2C
- 电源管理：AXP2101
- IMU：QMI8658（I2C 地址 `0x6B`）
- 蓝牙：BLE HID 键盘 + 相对鼠标复合设备
...
| 位置 | 丝印 | 实际用途 | 当前固件行为 |
| --- | --- | --- | --- |
| 最左 | IO10 | GPIO10 普通按键 | 键鼠配置2为鼠标右键；旧配置1按住为右 Option |
| 中间 | PWR | AXP2101 电源键 | 金山 AI 和键鼠配置2切换息屏/亮屏；旧配置1为退格；长按仍可能触发电源行为 |
| 最右 | BOOT | GPIO9 / BOOT 键 | 键鼠配置2为鼠标左键；旧配置1单击为回车 |
...
- 设备名：`XiaoZhi KB`
- HID 类型：Keyboard + Relative Mouse（同一连接，Report ID 1 / 2）
- VID / PID：`0x16C0 / 0x05DF`
- 配对方式：Just Works，无 PIN

选择界面的 `Keyboard + Mouse` 直接进入配置2：

> 说明：键盘与空鼠现在只有一套运行配置。旧 NVS 中的 profile 1 会在启动时
> 自动按配置2运行，不需要擦除 NVS；配置1 的旧键位代码仅为迁移兼容保留。

- **配置1**：保持原有键位。
- **配置2（菜单默认）**：保留九宫格触摸键盘，并在同一 BLE 连接上直接叠加 QMI8658 空鼠。

配置2 启动后会自动点亮九宫格触区图；短按 PWR 可在亮屏和暗屏之间切换。
...
| 最左键 GPIO10 按住 | 鼠标右键 |
| PWR 单击 | 切换 AMOLED 亮屏 / 暗屏 |
| 触摸屏九宫格 | 继续发送原有方向键、回车、退格和修饰键 |

空鼠数据链没有重新发明 IMU 驱动和姿态融合：

- QMI8658 使用 MIT 许可的 [SensorLib](https://github.com/lewisxhe/SensorLib)，固定到 commit `baa3e0b83c256b74d9870a95d96d55595946926c`，复用触摸/PMIC 已创建的 I2C0。
- 陀螺仪以 224 Hz ODR 和硬件低通工作，50 Hz 固定任务读取和发送；SensorLib 在检测到静止时动态更新陀螺仪零偏。
- 指针直接使用角速度，避免绝对姿态融合引入额外延迟和长期漂移；软件滤波沿用 BSD-3-Clause [One Euro Filter](https://github.com/casiez/OneEuroFilterArduino) 思路，并叠加连续死区、低速精细/高速加速曲线和分数像素累计。
- 空鼠在 BLE 连接期间持续工作；触摸屏同时保留键盘九宫格，不需要切换模式或占用触摸区。
- BLE HID 使用 ESP-IDF 自带的 Apache NimBLE 后端，通知路径非阻塞；连接后请求 15–30 ms interval 和 6 秒 supervision timeout，避免高频空鼠报告堵塞 BLE 事件任务并演变成断连或卡死。通知缓冲暂时忙时仍会退避 100 ms。
- 未连接时持续广播 `XiaoZhi KB`，不会沿用 ESP-IDF HID 示例的 180 秒广播超时；连接断开后自动恢复广播。
- 广播数据和连接后的 GAP Device Name 都固定为 `XiaoZhi KB`，不会在 macOS 已连接列表里变成 NimBLE 默认名 `nimble`。
- HID 使用 Just Works 的 Security Mode 1 Level 2（加密、无 PIN）并将 bond key 持久化到 NVS；连接建立后主动发起安全流程，避免 macOS 只建立临时 GATT 链路而一直显示“连接中”。

升级到含鼠标 Report 的固件后，macOS 可能仍缓存旧 HID 描述符。若只看到键盘功能，请先在系统蓝牙设置中忽略 `XiaoZhi KB`，再重新配对。当前实现已通过主机回归和 ESP32-C6 完整构建；指针轴向、灵敏度和手感仍需在目标实机上完成最终验收。
...
- `main/apps/ble_keyboard/air_mouse_motion.cc`
- `main/apps/ble_keyboard/one_euro_filter.cc`
...
- PWR 短按只切换 AMOLED 息屏/亮屏；息屏期间录音、播放、Wi-Fi、WSS、Agent 处理和 SD 持久化继续运行。触摸不会唤醒屏幕，再短按一次 PWR 才亮屏并重绘当前助手状态；长按仍由 AXP2101 硬件管理。
...
3. 在屏幕上点击想进入的应用：`XiaoZhi AI` / `Jinshan AI` / `Keyboard + Mouse`。
4. 设备写入 NVS 并重启，进入对应模式。
5. **金山 AI**：点 `点击说话` 开始、说完点 `发送`，turn 存到 `/sdcard/agent/`；联网后 AI 回复落卡并自动播放。PWR 短按息屏，再短按亮屏；左上角 `MENU` 长按 2 秒退出。
6. **键盘/空鼠模式**：在 macOS 蓝牙设置中连接 `XiaoZhi KB`，然后测试：
   - 启动后九宫格触区图自动亮起；转动设备移动指针，BOOT 为鼠标左键，GPIO10 为鼠标右键，PWR 切换屏幕。
   - 左下角退格，右上角回车，中间按住左 Command，右下角按住右 Option，方向触区保持不变。
   - 触摸屏左上角长按 2 秒返回选择界面。
...
- 点 `Keyboard + Mouse` 进入同一套键盘/空鼠配置（配置2）。
...
c++ -std=c++17 -Wall -Wextra -Werror \
  -I main/apps/ble_keyboard \
  main/apps/ble_keyboard/keyboard_zone_display_state_test.cc \
  main/apps/ble_keyboard/keyboard_zone_display_state.cc \
  -o /tmp/keyboard_zone_display_state_test && /tmp/keyboard_zone_display_state_test

c++ -std=c++17 -Wall -Wextra -Werror \
  -I main/apps/ble_keyboard \
  main/apps/ble_keyboard/air_mouse_motion_test.cc \
  main/apps/ble_keyboard/air_mouse_motion.cc \
  main/apps/ble_keyboard/one_euro_filter.cc \
  -o /tmp/air_mouse_motion_test && /tmp/air_mouse_motion_test
...
PWR 不是普通 GPIO，而是接在 AXP2101 PMIC 上的电源键。固件主要通过 AXP2101 短按中断识别单击，键鼠模式还使用松键沿作为硬件兼容兜底：金山 AI 和菜单默认的键鼠配置2切换 AMOLED 息屏/亮屏，旧键盘配置1映射为退格。金山 AI 息屏时后台语音、网络和 SD 工作不会暂停，触摸也不会唤醒。长按 PWR 仍可能触发硬件电源行为，调试和日常使用时不要把它当成长按功能键使用。
...
- 蓝牙 HID 键盘广播和连接。
- GPIO10 右 Option。
- BOOT 在配置1单击回车，在键鼠配置2作为鼠标左键。
- PWR 在配置1单击退格，在键鼠配置2切换 AMOLED 屏幕。
- 键鼠配置2已同时实现九宫格触摸键盘、持续陀螺仪移动、BOOT 左键和 GPIO10 右键；主机回归及固件构建通过，仍待真机确认轴向和手感。
- 触屏方向键和配置2四角/中心快捷触区。
- 蓝牙键盘模式下触屏左上角长按返回选择界面。
- 小智模式下 BOOT 长按返回选择界面。
- SD 卡（TF 卡）识别与挂载，三种模式均可用；开机读写自检。
- 键盘模式可把系统日志落盘到 `/sdcard/log/bootN.log`；小智和录音模式日志走 USB 串口。
- 独立金山 AI 模式：单一主按钮完成 `点击说话` / `发送` / `暂停` / `继续`，用户和 AI WAV 存到 `/sdcard/agent/`，完整回复自动播放；PWR 短按只切换 AMOLED 息屏/亮屏，左上角 `MENU` 长按退出。
