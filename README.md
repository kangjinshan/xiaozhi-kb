...
| 中间 | PWR | AXP2101 电源键 | 金山 AI 模式单击切换息屏/亮屏；键盘配置1单击为退格、配置2单击为 Tab；长按仍可能触发电源行为 |
...
PWR 不是普通 GPIO，而是接在 AXP2101 PMIC 上的电源键。固件通过读取 AXP2101 短按中断来识别单击：金山 AI 模式切换 AMOLED 息屏/亮屏，键盘配置1映射为退格，键盘配置2映射为 Tab。金山 AI 息屏时后台语音、网络和 SD 工作不会暂停，触摸也不会唤醒。长按 PWR 仍可能触发硬件电源行为，调试和日常使用时不要把它当成长按功能键使用。
