# XiaoZhi KB：小智语音 + 金山 AI 助手 + 蓝牙键盘/触控板
...
- **蓝牙键盘/触控板模式**：作为同一个 BLE HID 键鼠复合设备连接 MacBook；亮屏时整块触摸屏移动指针，暗屏时恢复九宫格键盘触区。
...
- 触摸：CST9217，I2C
- 电源管理：AXP2101
- IMU：QMI8658（I2C 地址 `0x6B`，键鼠模式不使用）
...
> 说明：键盘与鼠标只有一套运行配置。旧 NVS 中的 profile 1 会在启动时
> 自动按统一配置运行，不需要擦除 NVS；配置1 的旧键位代码仅为迁移兼容保留。

启动后自动亮屏并显示九宫格按键示意。PWR 不只控制显示，也在两种触摸输入方式之间切换：

| 输入 | 输出 |
| --- | --- |
| 亮屏时拖动触摸屏 | 移动鼠标指针；点按不触发九宫格按键，也不模拟鼠标点击 |
| 暗屏时触摸九宫格位置 | 保持原键盘方向键、回车、退格和修饰键行为 |
| BOOT 按住 | 鼠标左键，可拖拽 |
| 最左键 GPIO10 按住 | 鼠标右键 |
| PWR 单击 | 亮屏触控板 / 暗屏键盘之间切换 |

暗屏键盘模式中，方向键和退格/回车触区支持按住重复发送，当前重复间隔约 120 ms；右下角右 Option 和中间左 Command 按触摸按住/松开处理。左上角 `MENU` 长按 2 秒返回开机选择界面。
...
| BOOT 按住 | 鼠标左键，可拖拽 |
| 最左键 GPIO10 按住 | 鼠标右键 |
| PWR 单击 | 关闭鼠标移动并进入暗屏键盘，或重新亮屏启用触控板 |

触控板数据链：

- CST9217 以 50 Hz 读取校准后的绝对坐标，再转换成相对 HID 位移；慢速拖动保持精细，快速拖动使用有界加速。
- 新触点只建立坐标锚点，因此轻点触摸屏不会产生九宫格按键、鼠标点击或光标跳变。
- PWR 切换亮暗屏后必须先抬起手指再接受新模式输入，防止亮屏拖动在切到暗屏后误触键盘。
- BLE HID 使用 ESP-IDF 自带的 Apache NimBLE 后端，通知路径非阻塞；连接后请求 15–30 ms interval 和 6 秒 supervision timeout。通知缓冲暂时忙时退避 100 ms，避免高频指针报告演变成断连或卡死。
...
- `main/apps/ble_keyboard/touchpad_motion.cc`
...
6. **键盘/触控板模式**：在 macOS 蓝牙设置中连接 `XiaoZhi KB`，然后测试：
   - 启动后九宫格示意图自动亮起；在任意位置拖动可移动指针，点按不会触发图上的键。BOOT 为鼠标左键，GPIO10 为鼠标右键。
   - 短按 PWR 暗屏后，触摸位置恢复原九宫格键盘：左下角退格、右上角回车、中间左 Command、右下角右 Option，以及四向方向键。
   - 暗屏键盘模式下，触摸屏左上角长按 2 秒返回选择界面。
...
- 点 `Keyboard + Mouse` 进入同一套键盘/触控板配置（配置2）。
...
c++ -std=c++17 -Wall -Wextra -Werror \
  -I main/apps/ble_keyboard \
  main/apps/ble_keyboard/touchpad_motion_test.cc \
  main/apps/ble_keyboard/touchpad_motion.cc \
  -o /tmp/touchpad_motion_test && /tmp/touchpad_motion_test
...
- 键鼠配置2已实现亮屏触控板、暗屏九宫格键盘、BOOT 左键和 GPIO10 右键；触摸指针带有界加速和跳点抑制。
- 触屏方向键和配置2四角/中心快捷触区。
...
- 键盘模式的九宫格 UI 在亮屏时只作位置示意，整屏触摸用于移动光标；暗屏后相同位置才发送键盘按键。
