...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    // XiaoZhi KB: Just Works (no PIN). Use SC_BOND without MITM so no passkey/numeric-compare is required.
    ...
    esp_ble_io_cap_t iocap = ESP_IO_CAP_NONE;//NoInputNoOutput -> Just Works, no PIN (XiaoZhi KB requirement)
